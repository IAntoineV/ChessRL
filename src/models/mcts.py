...
class Node:
    ...
    def best_child(self, exploration_constant: float) -> "Node":
        ...
        for child in self.children:
            ...
            score = float("inf") if child.visits == 0 else (child.value / child.visits) + exploration_constant * math.sqrt(math.log(self.visits) / child.visits)
            if score > best_score:
                best_score = score
                best_child = child
        return best_child
...
class MCTS:
    def __init__(self, critic=None, exploration_constant: float = 1.414):
        self.critic = critic  # Neural network for board evaluation (if available).
    ...
    def simple_evaluation(self, board: chess.Board) -> float:
        ...
        return material / 39.0  #normalization by sum of piece values to get a value between -1 and 1
